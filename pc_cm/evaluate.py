...
# def run(gt_dirs, pred_dirs, top_k=5):
#     def is_float(string):
#         try:
#             float(string)
#             return True
...
#             cds = []
#             for gt_mesh_path in tqdm(gt_dir.glob("*.stl")):
#                 predicts = [stl for iter_dir in range(1, iter_num + 1)
#                             for stl in (pred_dir / str(iter_dir) / str(gt_mesh_path.stem)).glob("*.stl")]
#                 predicts = sorted(predicts, key=lambda x: float(x.stem) if is_float(x.stem) else float("inf"))
#                 predicts = predicts[:top_k]
#                 metrics = [run_cd_single((str(gt_mesh_path), str(pred_mesh_path)))[0]
#                                for pred_mesh_path in predicts]
#                 metrics = [m for m in metrics if m is not None]
#                 if metrics:
#                     cds.append(min(metrics))
#             print(gt_dir)
#             print(f"iteration: {iter_num} \tmedian cd = {np.median(cds) * 1000:.4f}")
...
def run(gt_dirs, pred_dirs):
    ...
    for gt_dir, pred_dir in zip(gt_dirs, pred_dirs):
        ...
        for iter_num in range(1, num_iters + 1):
            ...
            # ir = sum((1 if pred is None else 0) for gt, pred in gt_pred_pairs) / len(gt_pred_pairs)
...
def parse_args():
    ...
    parser.add_argument('--dataset', required=True, type=str,
                        help='Test dataset directory with .stl files.')
    parser.add_argument('--pred_dir', required=True, type=str,
                        help='Directory with test predictions (the per-dataset subfolder written by test.py).')
...
if __name__ == '__main__':
    ...
    run(gt_dirs=[args.dataset], pred_dirs=[args.pred_dir])
