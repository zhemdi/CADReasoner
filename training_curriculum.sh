...
torchrun --standalone --nproc_per_node="$n_gpus" train_group.py --group="1" --run_dir="$run_dir" --buffer_dir="$buffer_dir" --dataset="$dataset" --checkpoint="$checkpoint" --per_device_train_batch_size="$per_device_train_batch_size" 2>&1 | tee "$run_dir/torchrun_log_1.log" || true
...
torchrun --standalone --nproc_per_node="$n_gpus" train_group.py --group="2" --run_dir="$run_dir" --buffer_dir="$buffer_dir" --dataset="$dataset" --checkpoint="$checkpoint" --per_device_train_batch_size="$per_device_train_batch_size" 2>&1 | tee "$run_dir/torchrun_log_2.log" || true
